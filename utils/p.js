...
var u = {
  ...
  s = function (b, f) {
    ...
    for (var s = 0; s < h.length; s++)
      if (h[s]) {
        var value = []
        for (var index in h[s].split("|")) {
          var item = h[s].split("|")[index]
          if (c = item.split(","), !c[2] || (c[3] ? (c[2] <= e && c[3] >= e) : (c[2] <= e))) {
            value.push(c[1])
          }
        }
        if (value.length > 0) {
  ...
  g = function (b) {
    ...
    cce.festivalAction = cce.festivalTodo[0] && cce.festivalTodo[0].value ? cce.festivalTodo[0].value : (cce.term == '' ? (cce.lunarDate == 1 ? cce.lMonth + '月' : cce.lDate) : cce.term)
    cce.sut = sut(cce.festivalAction)
    cce.homeTop = []
    cce.homeTop.push(...cce.festivalTodo)
    cce.homeTop.push(cce.term == '' ? undefined : {
      value: [cce.term]
    })
    cce.isHomeTop = cce.homeTop.map(item => {
